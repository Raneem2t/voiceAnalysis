...
# wav_loc = "/Users/raneem/Desktop/voice/speech.wav"
wav_loc = "/Users/raneem/Desktop/voice/noise/01_samples_trimmed_noise_reduced/03_truck_ctr_s.wav"
...
#
# write("fimmm.wav", data=audio_clip_band_limited,rate=rate )
...
def removeNoise(audio_clip, noise_clip, n_grad_freq=2, n_grad_time=4, n_fft=2048, win_length=2048, hop_length=512,
                n_std_thresh=1.5, prop_decrease=1.0, verbose=False, visual=False):
    ...
    smoothing_filter = np.outer(
        np.concatenate([np.linspace(0, 1, n_grad_freq + 1, endpoint=False), np.linspace(1, 0, n_grad_freq + 2)])[1:-1],
    ...
    # mask the signal
    sig_stft_db_masked = sig_stft_db * (1 - sig_mask) + np.ones(
        np.shape(mask_gain_dB)) * mask_gain_dB * sig_mask  # mask real
...
write("file.wav", data=output,rate=44100)
